    ...
    var body:some View{
        ZStack(alignment: .bottom, content:{
            ...
            if isWithText == true {}
                ...
                }).frame(width: 170, height: 200, alignment: .center).clipped().cornerRadius(20, antialiased: true).shadow(radius: 10).padding(20)
...
struct SongCell:View{
    ...
    var body:some View{
        NavigationLink(destination: PlayerView(album: album, song: song), label: {
            HStack{
                ZStack{
                    Circle().frame(width: 40,height: 40, alignment: .center).foregroundColor(.blue)
